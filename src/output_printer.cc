...
////////////////////////////////////////
  ...
  cerr << "total_hits " << total_hits << endl; 
  ...
  for(int i = 0; i < result.size() && begin + i < upper; ++ i) {
    ...
    for(int j = 0; j < result[i].size(); ++ j) {
      ...
      for(int k = 0; k < result[i][j].targets.size(); ++ k) {
        ...
        cerr <<rid<<"    "<<num_maps<<"    "<<result[i][j].q_begin << "    "<<result[i][j].targets.size()<<"   "<<result[i][j].targets[k].sid <<"    " <<result[i][j].targets[k].begin << "   " <<result[i][j].targets[k].end  <<"    " << result[i][j].targets[k].num_mismatch <<"    "<<result[i][j].targets[k].cigar << endl;
      ...
      for(int i = 0; i < num_maps; ++ i) {
       ...
       // cerr << "num targets: " << num_targets << endl;
        for(int j = 0; j < num_targets; ++ j) {
          ...
          cerr<<rid <<"   "<<num_maps<<"   "<<result[rid][i].q_begin<<"   "<< num_targets <<"    "<<result[rid][i].targets[j].sid <<"    "  <<result[rid][i].targets[j].begin  << "   " << result[rid][i].targets[j].end  <<"    " << result[rid][i].targets[j].num_mismatch <<"    "<<result[rid][i].targets[j].cigar <<endl;
          ++ current_hits;
        }
